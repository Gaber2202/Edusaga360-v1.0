...
const ALLOWED_ROLES = ['edusaga_superadmin', 'edusaga_staff', 'creator'];
...
export function AuthProvider({ children }) {
  ...
  const validateAccess = async (authUser) => {
    try {
      ...
      const hasRole = ALLOWED_ROLES.includes(appUser.role) || ALLOWED_ROLES.includes(appUser.user_role);

      if (!isPlatformOwner && !hasRole) {
        setAccessDenied(true);
        setLoading(false);
        return;
      }
