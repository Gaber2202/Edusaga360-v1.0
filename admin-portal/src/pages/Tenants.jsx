...
function UserRow({ user, onRefresh }) {
  ...
  return (
    <tr className="hover:bg-slate-50">
      ...
      <td className="px-4 py-2.5">
        {editing ? (
          <select className="h-7 rounded border border-slate-300 px-2 text-xs bg-white" value={role} onChange={(e) => setRole(e.target.value)}>
      ...
      <td className="px-4 py-2.5">
        <span className={`text-xs font-medium px-2 py-0.5 rounded-full capitalize ${STATUS_COLORS[user.status] || 'bg-slate-100 text-slate-500'}`}>{user.status || 'active'}</span>
      </td>
      <td className="px-4 py-2.5 text-xs text-slate-400">{user.last_sign_in_at ? format(new Date(user.last_sign_in_at), 'MMM d') : 'Never'}</td>
      <td className="px-4 py-2.5">
        <div className="flex items-center gap-1 justify-end">
          {editing ? (
            ...
            <>
              <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => setEditing(true)} title="Edit role"><Edit2 className="w-3.5 h-3.5 text-slate-500" /></Button>
        ...
        <div className="flex items-center gap-4 p-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {tenant.name_ar && tenant.name_en && <p className="text-sm text-slate-400">{tenant.name_ar}</p>}
              <Badge className={`text-xs ${PLAN_COLORS[tenant.plan_code || tenant.plan] || 'bg-slate-100 text-slate-600'}`}>{planLabel(tenant.plan_code || tenant.plan)}</Badge>
            </div>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {tenant.status === 'trial' && daysLeft != null && (
              <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${daysLeft < 0 ? 'bg-red-100 text-red-700' : daysLeft <= 7 ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600'}`}>
                {daysLeft < 0 ? `Expired ${-daysLeft}d ago` : `${daysLeft}d left`}
              </span>
            )}
            <Badge className={`text-xs capitalize ${STATUS_COLORS[tenant.status] || 'bg-slate-100 text-slate-700'}`}>{tenant.status?.replace('_', ' ')}</Badge>
            ...
            {tenant.status === 'trial' && (
              <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => post('/extend-trial', { days: 14 }, 'Trial extended +14 days')} title="Extend trial +14d"><Calendar className="w-4 h-4 text-slate-500" /></Button>
            )}
            <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => setEditMode(!editMode)} title="Edit"><Edit2 className="w-4 h-4 text-slate-500" /></Button>
            ...
            <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => setExpanded(!expanded)} title="Manage users"><Users className="w-4 h-4 text-blue-500" /></Button>
            ...
            <button onClick={() => setExpanded(!expanded)} className="text-slate-400 hover:text-slate-600">{expanded ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}</button>
          </div>
        </div>
        ...
        {editMode && (
          <div className="border-t border-slate-100 bg-slate-50 p-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
              {[
                ...
                <div key={key}>
                  ...
                  {type === 'select' ? (
                    <select className="w-full h-8 rounded border border-slate-200 px-2 text-xs bg-white" value={form[key]} onChange={(e) => setForm((f) => ({ ...f, [key]: e.target.value }))}>
                      {opts.map((o) => <option key={o} value={o}>{o}</option>)}
                    </select>
                  ) : (
                    <input type={type} className="w-full h-8 rounded border border-slate-200 px-2 text-xs" value={form[key]} onChange={(e) => setForm((f) => ({ ...f, [key]: e.target.value }))} />
                  )}
                </div>
              ))}
            </div>
            <div className="mb-3">
              ...
              <input type="text" className="w-full h-8 rounded border border-slate-200 px-2 text-xs" placeholder="e.g. Spoke to principal on June 8" value={form.notes} onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))} />
            </div>
        ...
        {expanded && (
          <div className="border-t border-slate-100">
            <div className="px-4 py-3 bg-slate-50 flex items-center justify-between">
              <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Users ({users.length})</p>
              <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => refetchUsers()}><RefreshCw className="w-3 h-3 me-1" /> Refresh</Button>
            </div>
            {users.length === 0 ? (
              <p className="text-xs text-slate-400 text-center py-4">No users in this school</p>
            ) : (
              <table className="w-full text-sm">
                <thead className="border-b border-slate-100">
                  <tr>{['Name / Email', 'Role', 'Status', 'Last login', ''].map((h) => <th key={h} className="text-left px-4 py-2 text-xs font-semibold text-slate-400">{h}</th>)}</tr>
                </thead>
