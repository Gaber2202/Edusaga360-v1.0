...
const SMOKE_TESTS = [
  ...
  },
  {
    ...
    nameAr: 'كيان الطالب',
    test: async () => {
      const students = await tenantQuery('students').select('*').order('-created_date', 1);
  ...
  },
  {
    ...
    nameAr: 'كيان الموظف',
    test: async () => {
      const employees = await tenantQuery('employees').select('*').order('-created_date', 1);
  ...
  },
  {
    ...
    test: async () => {
      const errors = await tenantQuery('system_errors').select('*').order('-timestamp', 1);
