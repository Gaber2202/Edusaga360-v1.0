  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-ink">{isRTL ? 'إدارة المكتبة' : 'Library Management'}</h1>
          <p className="text-sm text-muted-foreground">{isRTL ? 'فهرس الكتب، الاستعارة، والتقارير' : 'Book catalog, circulation & reports'}</p>
        </div>
        ...
        <TabsContent value="scan" className="mt-4">
          <div className="max-w-md mx-auto bg-white border rounded-2xl p-5 shadow-sm">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-semibold text-ink">{isRTL ? 'استعارة بالباركود — أقل من 5 ثوانٍ' : 'Barcode Checkout — Under 5 Seconds'}</h2>
            </div>
            <BarcodeCheckout
              books={books}
              students={students}
              ...
              <Table>
                ...
                <TableBody>
                  {isLoading ? (
                    ...
                    <TableRow><TableCell colSpan={7} className="text-center py-10 text-muted-foreground">{isRTL ? 'لا توجد كتب' : 'No books found'}</TableCell></TableRow>
                  ) : filteredBooks.map(b => (
                    <TableRow key={b.id}>
                      <TableCell>
                        <p className="font-medium text-sm">{b.title_ar}</p>
                        {b.title_en && <p className="text-xs text-muted-foreground">{b.title_en}</p>}
                        {b.isbn && <p className="text-xs text-muted-foreground font-mono">{b.isbn}</p>}
                      </TableCell>
                      <TableCell className="text-sm">{b.author}</TableCell>
                      <TableCell><span className="text-xs px-2 py-0.5 bg-sand-alt rounded">{LANG_OPTIONS.find(l => l.value === b.language)?.[isRTL ? 'ar' : 'en']}</span></TableCell>
                      ...
                      <TableCell className="text-xs text-muted-foreground">{b.shelf_location}</TableCell>
                      <TableCell>
                  ...
                  {loans.slice(0, 50).map(l => {
                    ...
                    return (
                      <TableRow key={l.id} className={overdue ? 'bg-red-50/30' : ''}>
                        ...
                        <TableCell className="text-xs text-muted-foreground">{l.checkout_date}</TableCell>
                        <TableCell>
                          <span className={`text-xs font-medium ${overdue ? 'text-red-600' : 'text-muted-foreground'}`}>{l.due_date}</span>
                          {overdue && <p className="text-xs text-red-500">{differenceInDays(new Date(), new Date(l.due_date))} {isRTL ? 'يوم تأخر' : 'days late'}</p>}
                        </TableCell>
                        <TableCell>
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${l.status === 'returned' ? 'bg-green-100 text-green-700' : overdue ? 'bg-red-100 text-red-700' : 'bg-najdi-50 text-najdi-900'}`}>
                            {l.status}
                          </span>
                        </TableCell>
                        <TableCell>
                ...
                <Table>
                  ...
                  <TableBody>
                    {overdueLoans.map(l => {
                      ...
                      return (
                        <TableRow key={l.id}>
                          <TableCell><p className="font-medium text-sm">{l.student_name}</p><p className="text-xs text-muted-foreground">{l.grade}</p></TableCell>
              ...
              {bookSearch && !checkoutData.book_id && (
                <div className="border rounded-lg max-h-40 overflow-y-auto">
                  {books.filter(b => b.available_copies > 0 && (b.title_ar?.includes(bookSearch) || b.isbn?.includes(bookSearch))).slice(0, 6).map(b => (
                    <button key={b.id} onClick={() => { setCheckoutData(d => ({ ...d, book_id: b.id })); setBookSearch(b.title_ar); }} className="w-full text-start px-3 py-2 hover:bg-sand text-sm border-b last:border-0">
              ...
              {studentSearch && !checkoutData.student_id && (
                <div className="border rounded-lg max-h-40 overflow-y-auto">
                  {students.filter(s => s.name_ar?.includes(studentSearch) || s.name_en?.toLowerCase().includes(studentSearch.toLowerCase())).slice(0, 6).map(s => (
                    <button key={s.id} onClick={() => { setCheckoutData(d => ({ ...d, student_id: s.id, student_name: s.name_ar })); setStudentSearch(s.name_ar); }} className="w-full text-start px-3 py-2 hover:bg-sand text-sm border-b last:border-0">
                      {s.name_ar} <span className="text-muted-foreground ms-2">{s.grade}</span>
                    </button>
                  ))}
                </div>
              )}
