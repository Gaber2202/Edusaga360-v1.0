...
export default function StudentTags() {
  ...
  const { data: tags = [], isLoading } = useTenantQuery(
    ['studentTags', tenantId],
    () => fetchData(tenantQuery('student_tags').select('*').match(tenantFilter()).order('created_at', { ascending: false })),
