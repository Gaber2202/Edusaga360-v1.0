    ...
    (async () => {
      try {
        ...
        if (isPlatformOwner(me)) {
          const all = await supabase.from('tenants').select('*').order('-created_date', 200);
          setTenants(all || []);
        }
      } finally {
        setLoading(false);
      ...
      <div className="min-h-[60vh] flex items-center justify-center p-6">
        <Card className="max-w-md w-full">
          <CardContent className="pt-6 text-center space-y-3">
            ...
            <p className="text-sm text-slate-500">هذه الصفحة متاحة لمشرف المنصة (Super Admin) فقط.</p>
          </CardContent>
        </Card>
      </div>
    );
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-6" dir="rtl">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">اختر مؤسسة ثم اضغط زرع لإضافة بيانات شاملة (طلاب، موظفين، فواتير، حضور...).</p>
        </div>
      </div>
        ...
        <CardContent className="space-y-4">
          ...
          {selectedTenant && (
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700 space-y-1">
              <div><span className="text-slate-500">الكود:</span> {selectedTenant.tenant_code || '—'}</div>
              <div><span className="text-slate-500">الحالة:</span> <Badge variant="outline">{selectedTenant.status || '—'}</Badge></div>
            </div>
          )}
          ...
          <Button onClick={handleSeed} disabled={seeding || !selectedTenantId} className="w-full bg-blue-600 hover:bg-blue-700">
            {seeding ? (
        ...
        <Card>
          ...
          <CardContent>
            {Object.keys(lastResult).length === 0 ? (
              <p className="text-sm text-slate-500">لم يتم زرع سجلات جديدة (البيانات موجودة بالفعل لهذه المؤسسة).</p>
            ) : (
