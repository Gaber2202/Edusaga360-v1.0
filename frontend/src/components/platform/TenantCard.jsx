...
const STATUS_STYLES = {
  trial:     'bg-najdi-50 text-najdi-900 border-najdi-100',
  active:    'bg-emerald-100 text-emerald-700 border-emerald-200',
  past_due:  'bg-amber-100 text-amber-700 border-amber-200',
  suspended: 'bg-red-100 text-red-700 border-red-200',
  cancelled: 'bg-sand-alt text-muted-foreground border-border',
};
...
const PLAN_STYLES = {
  starter:    'bg-sand-alt text-muted-foreground',
    ...
    <div className="bg-white rounded-xl border border-border p-5 hover:shadow-md transition-shadow">
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          {tenant.logo_url
            ? <img src={tenant.logo_url} alt={name} className="w-10 h-10 rounded-lg object-cover border border-border" />
            : <div className="w-10 h-10 rounded-lg bg-sand-alt flex items-center justify-center"><Building2 className="w-5 h-5 text-muted-foreground" /></div>
          }
          <div>
            <h3 className="font-semibold text-ink text-sm">{name}</h3>
            <p className="text-xs text-muted-foreground">{tenant.tenant_code}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        <div className="text-center p-2 bg-sand rounded-lg">
          <div className="flex items-center justify-center gap-1 mb-1"><Users className="w-3 h-3 text-muted-foreground" /></div>
          <div className="text-sm font-bold text-ink">{tenant.current_employees || 0}<span className="text-xs text-muted-foreground">/{tenant.max_employees || '∞'}</span></div>
          <div className="text-xs text-muted-foreground">{isRTL ? 'موظف' : 'Employees'}</div>
        </div>
        <div className="text-center p-2 bg-sand rounded-lg">
          <div className="flex items-center justify-center gap-1 mb-1"><GraduationCap className="w-3 h-3 text-muted-foreground" /></div>
          <div className="text-sm font-bold text-ink">{tenant.current_students || 0}<span className="text-xs text-muted-foreground">/{tenant.max_students || '∞'}</span></div>
          <div className="text-xs text-muted-foreground">{isRTL ? 'طالب' : 'Students'}</div>
        </div>
        <div className="text-center p-2 bg-sand rounded-lg">
          <div className="flex items-center justify-center gap-1 mb-1"><Bot className="w-3 h-3 text-muted-foreground" /></div>
          <div className="text-sm font-bold text-ink">{tenant.yamen_ai_used_this_month || 0}<span className="text-xs text-muted-foreground">/{tenant.yamen_ai_monthly_limit || '∞'}</span></div>
          <div className="text-xs text-muted-foreground">{isRTL ? 'يامن AI' : 'Yamen AI'}</div>
        </div>
      </div>
      ...
      {/* Admin */}
      {tenant.admin_email && (
        <div className="mt-2 text-xs text-muted-foreground truncate">{tenant.admin_email}</div>
      )}
    </div>
  );
}