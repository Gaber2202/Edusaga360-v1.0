  ...
  async notifyPaymentReceived(payment, invoice) {
    try {
      await callApi('/api/functions/createNotificationEvent', {
        ...
        message_ar: `تم استلام دفعة بمبلغ ${payment.amount} ر.س للفاتورة ${invoice.invoice_number}`,
        message_en: `Payment of ${payment.amount} SAR received for invoice ${invoice.invoice_number}`,
        entity_type: 'Payment',
