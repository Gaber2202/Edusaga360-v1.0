...
export default function QuickActionTile({ label, icon: Icon, href, accentIndex = 0 }) {
  ...
  return (
    <Link to={href}>
      <div className="bg-white p-4 rounded-xl flex flex-col items-center gap-2 border border-slate-200 hover:border-slate-300 hover:shadow-lg hover:-translate-y-1 transition-all duration-200 group cursor-pointer">
        <div className={`w-10 h-10 rounded-lg ${accent} flex items-center justify-center transition-colors`}>
          <Icon className="w-5 h-5" />
        </div>
        <span className="text-slate-600 text-xs font-semibold text-center leading-tight">{label}</span>
      </div>
    </Link>
  );
}