...
export default function GovComplianceDashboard() {
  ...
  const { data: iqamas = [] } = useQuery({ enabled: false /* iqama_records table not built */, queryKey: ['iqamas'], queryFn: () => fetchData(tenantQuery('iqama_records').select('*').order('created_at', { ascending: false })), initialData: [] });
  const { data: visas = [] } = useQuery({ enabled: false /* visa_records table not built */, queryKey: ['visas'], queryFn: () => fetchData(tenantQuery('visa_records').select('*').order('created_at', { ascending: false })), initialData: [] });
  const { data: violations = [] } = useQuery({ enabled: false /* govi_violations table not built */, queryKey: ['violations'], queryFn: () => fetchData(tenantQuery('govi_violations').select('*').order('created_at', { ascending: false })), initialData: [] });
  const { data: govDocs = [] } = useQuery({ enabled: false /* gov_documents table not built */, queryKey: ['govDocs'], queryFn: () => fetchData(tenantQuery('gov_documents').select('*').order('created_at', { ascending: false })), initialData: [] });
  const { data: mudad = [] } = useQuery({ enabled: false /* mudad_submissions table not built */, queryKey: ['mudad'], queryFn: () => fetchData(tenantQuery('mudad_submissions').select('*').order('created_at', { ascending: false })), initialData: [] });
  const { data: gosiRecords = [] } = useQuery({ enabled: false /* gosi_records table not built */, queryKey: ['gosiRecords'], queryFn: () => fetchData(tenantQuery('gosi_records').select('*').order('created_at', { ascending: false })), initialData: [] });
