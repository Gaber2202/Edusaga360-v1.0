...
export default function IqamaServices() {
  ...
  const { data: iqamas = [], isLoading } = useQuery({ enabled: false /* iqama_records table not built */, queryKey: ['iqamas'], queryFn: () => fetchData(tenantQuery('iqama_records').select('*').order('created_at', { ascending: false })), initialData: [] });
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-xl">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              <Label>{isRTL ? `الرسوم (${getCurrencySymbol(tenant?.localization, isRTL)})` : `Fee (${getCurrencySymbol(tenant?.localization, isRTL)})`}</Label>
