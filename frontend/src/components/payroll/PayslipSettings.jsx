...
export default function PayslipSettings({ open, onClose }) {
  ...
  const [settings, setSettings] = useState({
    ...
    auto_channels: ['email'],
    max_retries: 3,
    email_template_ar: 'مرحباً {{employee_name}},\n\nقسيمة راتبك لشهر {{period}} جاهزة.\n\nصافي الراتب: {{net_salary}} {{currency_code}}',
    email_template_en: 'Dear {{employee_name}},\n\nYour payslip for {{period}} is ready.\n\nNet Salary: {{net_salary}} {{currency_code}}',
    whatsapp_template_ar: 'قسيمة راتب {{period}} جاهزة. صافي الراتب: {{net_salary}} {{currency_code}}',
    whatsapp_template_en: 'Payslip for {{period}} is ready. Net: {{net_salary}} {{currency_code}}',
  ...
  const { data: existingSettings } = useQuery({ enabled: false /* payslip_settings table not built */, queryKey: ['payslipSettings', tenantId], queryFn: async () => {
