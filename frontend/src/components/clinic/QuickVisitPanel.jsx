/**
 * QuickVisitPanel — nurse logs visit + dispenses medication + notifies parent in <60s
 */
...
export default function QuickVisitPanel({ students, healthRecords, getTenantIdForCreate, onSaved }) {
  ...
  const allergyList = normalizeAllergies(healthRecord, selectedStudent);
  const critical = isCriticalHealth(healthRecord, selectedStudent);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const now = new Date();
      const visitTime = format(now, 'HH:mm');
      const visitPayload = {
        ...(tid && { tenant_id: tid }),
        ...
        medication_dispensed: med || undefined,
        medication_dose: dose || undefined,
        outcome,
        parent_notified: notifyParent,
        parent_notified_at: notifyParent ? now.toISOString() : null,
      };

      const { error } = await tenantQuery('clinic_visits').insert(visitPayload);
      ...
      fireEvent(
        ...
        buildClinicVisitEvent({
          ...
          visit: {
            ...visitPayload,
            treatment_given: med,
            complaint: COMPLAINTS.find((c) => c.value === complaint)?.en || complaint,
          },
          tenantId: tid,
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3 p-3 bg-red-50 rounded-xl border border-red-100">
            ...
            {critical && (
              <div className="ms-auto flex items-center gap-1 text-xs text-red-700 bg-red-200 px-2 py-1 rounded-lg">
                ...
                {criticalNote(healthRecord, selectedStudent)?.slice(0, 30) || (isRTL ? 'حالة حرجة' : 'Critical')}
              </div>
            )}
          </div>
          ...
          {allergyList.length > 0 && (
            <div className="flex flex-wrap gap-1 px-1">
              {allergyList.map((a, i) => (
                <span key={`${a.name}-${i}`} className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-medium">
                  {a.name}
                </span>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 gap-2">
              {OUTCOMES.map((o) => (
                <button
                  ...
                  onClick={() => {
                    setOutcome(o.value);
                    if (o.value !== 'returned_to_class') setNotifyParent(true);
                  }}
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {isRTL ? 'إرسال رسالة واتساب لولي الأمر' : 'Queue WhatsApp message to guardian'}
                </p>
              </div>
            </div>
            <button
              type="button"
              onClick={() => setNotifyParent((n) => !n)}
