...
const STAGES = [
  { key: 'applied',              ar: 'تقديم',            en: 'Applied',      color: 'bg-sand-alt text-ink',    border: 'border-border' },
  { key: 'screening',            ar: 'فرز',              en: 'Screening',    color: 'bg-najdi-50 text-najdi-900',      border: 'border-najdi-100' },
          ...
          {STAGES.map(stage => (
            <div key={stage.key} className="w-60 flex-shrink-0">
              <div className={`px-3 py-2 rounded-t-xl font-semibold text-xs flex items-center justify-between ${stage.color}`}>
                ...
                <Badge className="bg-white/70 text-ink text-xs font-bold">{stageApplicants(stage.key).length}</Badge>
              </div>
              <div className={`bg-sand border ${stage.border} rounded-b-xl p-2 space-y-2 min-h-[240px]`}>
                {stageApplicants(stage.key).map(app => (
                  <Card
                    key={app.id}
                    className="cursor-pointer hover:shadow-md transition-all border border-border bg-white"
                    onClick={() => setSelectedApplicant(app)}
                  >
                    <CardContent className="p-3 space-y-2">
                      <p className="font-semibold text-sm text-ink leading-tight">{app.full_name_ar}</p>
                      {app.full_name_en && <p className="text-xs text-muted-foreground">{app.full_name_en}</p>}
                      {app.job_title && <p className="text-xs text-muted-foreground italic">{app.job_title}</p>}
                      ...
                      <div className="flex gap-1 flex-wrap pt-1" onClick={e => e.stopPropagation()}>
                        {stage.key === 'applied' && (
                          <Button size="sm" className="h-6 text-xs px-2 bg-najdi-700 hover:bg-najdi-900"
        ...
        <Dialog open={!!selectedApplicant} onOpenChange={() => { setSelectedApplicant(null); setAiSuggestion(''); setEditMode(false); }}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 flex-wrap">
                ...
                {selectedApplicant.applicant_number && (
                  <span className="text-muted-foreground font-normal text-sm">— {selectedApplicant.applicant_number}</span>
                )}
                <Badge className={STAGES.find(s => s.key === selectedApplicant.status)?.color || 'bg-sand-alt'}>
            ...
            <div className="space-y-4 text-sm">
              {editMode ? (
                <div className="grid grid-cols-2 gap-3 bg-sand p-3 rounded-lg">
                  ...
                  <div className="col-span-2">
                    <Button className="w-full gap-2 bg-najdi-700 hover:bg-najdi-900" onClick={handleSaveEdit} disabled={savingEdit}>
              ...
              <div className="grid grid-cols-2 gap-3 bg-sand p-3 rounded-lg">
                <div><Label className="text-muted-foreground">{isRTL ? 'البريد' : 'Email'}</Label><p>{selectedApplicant.email || '-'}</p></div>
              ...
              {selectedApplicant.interview_scheduled_at && (
                <div className={`rounded-lg p-3 border flex items-start gap-3 ${selectedApplicant.interview_mode === 'virtual' ? 'bg-najdi-50 border-najdi-100' : 'bg-amber-50 border-amber-200'}`}>
                  {selectedApplicant.interview_mode === 'virtual' ? <Video className="w-4 h-4 text-najdi-700 flex-shrink-0 mt-0.5" /> : <MapPin className="w-4 h-4 text-amber-600 flex-shrink-0 mt-0.5" />}
                  <div>
                    <p className={`font-semibold text-xs mb-0.5 ${selectedApplicant.interview_mode === 'virtual' ? 'text-najdi-900' : 'text-amber-800'}`}>
                    ...
                    {selectedApplicant.interview_location && <p className="text-xs text-muted-foreground mt-0.5">{selectedApplicant.interview_location}</p>}
                    {selectedApplicant.interview_notes && <p className="text-xs text-muted-foreground mt-0.5 italic">{selectedApplicant.interview_notes}</p>}
                  </div>
                </div>
              )}
              ...
              {selectedApplicant.interview_score > 0 && (
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                  ...
                  <div className="grid grid-cols-4 gap-2 text-center text-xs">
                    <div><p className="text-lg font-bold text-najdi-700">{selectedApplicant.interview_technical_score || 0}</p><p className="text-muted-foreground">{isRTL ? 'تقني' : 'Technical'}</p></div>
                    <div><p className="text-lg font-bold text-purple-600">{selectedApplicant.interview_communication_score || 0}</p><p className="text-muted-foreground">{isRTL ? 'تواصل' : 'Comm.'}</p></div>
                    <div><p className="text-lg font-bold text-emerald-600">{selectedApplicant.interview_culture_score || 0}</p><p className="text-muted-foreground">{isRTL ? 'انسجام' : 'Culture'}</p></div>
                    <div><p className="text-lg font-bold text-amber-600">{selectedApplicant.interview_score}</p><p className="text-muted-foreground">{isRTL ? 'متوسط' : 'Avg'}/5</p></div>
                  </div>
                </div>
              )}
                ...
                return (
                  <div className="flex items-center gap-2 border-t pt-3">
                    ...
                    {nextStage && selectedApplicant.status !== 'rejected' && (
                      <Button size="sm" className="flex-1 gap-1 text-xs bg-najdi-700 hover:bg-najdi-900" disabled={movingId === selectedApplicant.id}
        ...
        <Dialog open={!!pendingMove} onOpenChange={() => setPendingMove(null)}>
          <DialogContent className="max-w-md">
            ...
            <DialogFooter className="mt-4 gap-2">
              ...
              <Button className="bg-najdi-700 hover:bg-najdi-900" onClick={() => {
          ...
          <DialogContent className="max-w-md">
            ...
            <div className="space-y-4 text-sm">
              <div className="bg-sand rounded-lg p-3">
                <p className="font-semibold text-ink">{showScheduleInterview.full_name_ar}</p>
                {showScheduleInterview.full_name_en && <p className="text-muted-foreground text-xs">{showScheduleInterview.full_name_en}</p>}
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={`flex items-center gap-2 p-3 rounded-lg border-2 transition-all ${scheduleForm.interview_mode === 'on_site' ? 'border-amber-500 bg-amber-50 text-amber-800' : 'border-border text-muted-foreground hover:border-border'}`}
                  >
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 p-3 rounded-lg border-2 transition-all ${scheduleForm.interview_mode === 'virtual' ? 'border-najdi-500 bg-najdi-50 text-najdi-900' : 'border-border text-muted-foreground hover:border-border'}`}
                  >
              ...
              <div className="bg-najdi-50 border border-najdi-100 rounded-lg p-3 flex items-start gap-2 text-xs text-najdi-900">
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-amber-50 rounded-lg p-3 text-center border border-amber-200">
              <p className="text-xs text-muted-foreground mb-1">{isRTL ? 'متوسط النتيجة' : 'Average Score'}</p>
              <p className="text-3xl font-bold text-amber-600">{((interviewScore.technical + interviewScore.communication + interviewScore.cultural) / 3).toFixed(1)}<span className="text-base text-muted-foreground">/5</span></p>
            </div>
          </div>
          <DialogFooter>
