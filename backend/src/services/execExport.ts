...
function renderCHRO(data: any, isRTL: boolean): string {
  ...
  return [
    ...
    section(isRTL ? 'الالتزام الحكومي' : 'Government Compliance', `<p>${isRTL ? 'الفوترة الإلكترونية / ضريبة القيمة المضافة' : 'E-Invoicing / VAT'}: ${comp.einvoicing?.status || '—'}<br>${isRTL ? 'حماية الأجور' : 'Wage Protection'}: ${comp.mudad?.status || '—'}<br>${isRTL ? 'التأمينات الاجتماعية' : 'Social Insurance'}: ${comp.gosi?.status || '—'}<br>${isRTL ? 'بوابة العمل' : 'Labor Portal'}: ${comp.qiwa?.status || '—'}</p>`, isRTL),
  ].join('');
}
