 ...
 *
 * This is a generic calendar algorithm, not a Saudi-specific rule. It uses the ICU
...
/** True if the given Gregorian date falls in Ramadan (Umm al-Qura). */
export function isRamadan(input: Date | string): boolean {
  const h = gregorianToHijri(input);
  return h.month === 9;
}
